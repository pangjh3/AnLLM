...
def create_prompt(instruct, input_data, template="prompt_no_input"):
    if "###" in instruct:
        ...
        instruct =  instruct
        hint = ""
    if template == "prompt_input" or template == "prompt_input_ac":
...
if __name__ == "__main__":
    ...
    parser.add_argument('--length', '-l', type=int, default=512, help='length of the output text')
    ...
    with open(output_file, 'w', encoding='utf-8') as fo,open(output_file+".hyp", 'w', encoding='utf-8') as fo2:
        for i in range(0, len(prompt), batch):
            ...
            model.model.acindex=[]
            model.model.offset=0
        print("allkvl: {},count: {}, avgkvl: {}".format(allkvl, count, allkvl/count))