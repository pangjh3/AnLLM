...
def llamamodel_forwardforinfer_realtime_asan(
    self,
    ...
    pastanchorpkv = [_ for _ in self.acindex]
    ...
    if torch.sum(indextruefalse) != 0:
        ...
        #     start=ix+1
        for i, ix in enumerate(acindex):
            mask[ix+1:,start:ix] = torch.finfo(datatype).min
            start=ix+1
            mask[ix, acindex[:i]] = torch.finfo(datatype).min
        ...
        pastpkvi.extend([i + past_key_values_length for i in range(maxacindex+1, seq_length)])

        self.acindex.extend([i+len(self.acindex) for i in range(len(acindex))])
 

    if past_key_values_length > 0:
        inputtopastkvmask=torch.zeros(seq_length, past_key_values_length, dtype=datatype, device=datadevice)
        if acindex is not None:

            # aa = [[elem] * len(pastanchorpkv) for elem in acindex]
            # bb = [pastanchorpkv for _ in acindex]

            at = torch.tensor(acindex)  # convert to tensor
            bt = torch.tensor(pastanchorpkv)  # convert to tensor
            aa = at.unsqueeze(-1).repeat(1, bt.shape[0]).tolist()
            bb = bt.repeat(at.shape[0], 1).tolist()
            inputtopastkvmask[aa,bb]=torch.finfo(datatype).min

        mask = torch.cat([inputtopastkvmask, mask], dim=-1)
    ...
    for idx, decoder_layer in enumerate(self.layers):
        ...
        if use_cache:
            ...
            if hasanchor:
                next_decoder_cache += ((pkvi[0][:,:,pastpkvi,:].contiguous(), pkvi[1][:,:,pastpkvi,:].contiguous()),)
            else:
                # print(pkvi)
...
def llamamodel_forwardforinfer_onlykvcache_realtime(
    self,
    input_ids: torch.LongTensor = None,
    attention_mask: Optional[torch.Tensor] = None,
    past_key_values: Optional[List[torch.FloatTensor]] = None,
...
def llamamodel_forward_onlyfortraining(
    self,
    input_ids: torch.LongTensor = None,
    attention_mask: Optional[torch.Tensor] = None,
    past_key_values: Optional[List[torch.FloatTensor]] = None,
...
def replace_llama_forward_fortraining(ac_tokenid, pad_tokenid=PAD_TOKEN_ID):
    global AC_TOKEN_ID, PAD_TOKEN_ID
    AC_TOKEN_ID = ac_tokenid
    PAD_TOKEN_ID = pad_tokenid
    transformers.models.llama.modeling_llama.LlamaModel.forward = llamamodel_forward_onlyfortraining


def replace_llama_forward_forinference(ac_tokenid, pad_tokenid=PAD_TOKEN_ID):
    global AC_TOKEN_ID, PAD_TOKEN_ID
    AC_TOKEN_ID = ac_tokenid
    PAD_TOKEN_ID = pad_tokenid
    transformers.models.llama.modeling_llama.LlamaModel.forward = llamamodel_forwardforinfer_onlykvcache_realtime




def replace_llama_forward_forinference_withasan(ac_tokenid, pad_tokenid=PAD_TOKEN_ID):
    global AC_TOKEN_ID, PAD_TOKEN_ID
    AC_TOKEN_ID = ac_tokenid
    PAD_TOKEN_ID = pad_tokenid
    transformers.models.llama.modeling_llama.LlamaModel.forward = llamamodel_forwardforinfer_realtime_asan