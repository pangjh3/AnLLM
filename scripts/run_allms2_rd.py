...
DEFAULT_PJH_S_TOKEN = "<acs>"
DEFAULT_PJH_E_TOKEN = "<ace>"
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if "validation" not in raw_datasets.keys():
            ...
            raw_datasets["train"] = load_dataset(
                data_args.dataset_name,
                data_args.dataset_config_name,
                split=f"train[{data_args.validation_split_percentage}%:]" if training_args.do_eval else f"train[:]",
                cache_dir=model_args.cache_dir,
                use_auth_token=True if model_args.use_auth_token else None,
                streaming=data_args.streaming,
            )
    ...
    tokenizer_kwargs = {
        ...
        "revision": model_args.model_revision,
        "use_auth_token": True if model_args.use_auth_token else None,
        "model_max_length": model_args.context_length if model_args.context_length is not None else 4096
    }
    ...
    donewanchor=False
    # newanchortoken = None
    if model_args.anchor_symbols is not None:
        vocab = tokenizer.get_vocab()
        # def is_word_in_vocab(word):
        #     return word in vocab
        achorid=[]
        anchor_syms=model_args.anchor_symbols.strip().split('|')
        assert len(anchor_syms) == 2
        assert anchor_syms[0] == DEFAULT_PJH_S_TOKEN
        assert anchor_syms[1] == DEFAULT_PJH_E_TOKEN
        for ans in anchor_syms:
            if ans in vocab:
                achorid.append(tokenizer.convert_tokens_to_ids(ans))
                logger.info("The old {} token as an anchor".format(ans))
            else:
                donewanchor=True
                # newanchortoken=ans
            
                num_added_toks = tokenizer.add_tokens([ans])
                logger.info("We have added new {} token {} as an anchor".format(num_added_toks, ans))

                embedding_size = model.get_input_embeddings().weight.shape[0]
                if len(tokenizer) > embedding_size:
                    model.resize_token_embeddings(len(tokenizer))
                newtokenid=tokenizer.convert_tokens_to_ids(ans)
                achorid.append(newtokenid)
        ...
        replace_allmtrainingforward(achorid[0], achorid[1], tokenizer.convert_tokens_to_ids(tokenizer.pad_token))
    else:
        logger.info("No Anchor! No ALLM!")
    ...
    def add_ac(example):
        ...
        # metainfo = json.loads(example['meta'].replace('\'','\"'))
        # if 'language' in metainfo:
        #     languageindex = metainfo['language']
        # else:
        #     languageindex = 'en'
        ...
        # elif  languageindex == 'it':
        #     sent_tokenizer = nltk.data.load('tokenizers/punkt/italian.pickle')
        # elif  languageindex == 'es':
        #     sent_tokenizer = nltk.data.load('tokenizers/punkt/spanish.pickle')
        # elif  languageindex == 'fr':
        #     sent_tokenizer = nltk.data.load('tokenizers/punkt/french.pickle')
        # elif  languageindex == 'de':
        #     sent_tokenizer = nltk.data.load('tokenizers/punkt/german.pickle')
        # else:
        #     sent_tokenizer = nltk.data.load('tokenizers/punkt/english.pickle')
        # sent_tokenizer = nltk.data.load('tokenizers/punkt/english.pickle')
        textsents = sent_tokenizer.tokenize(example[text_column_name])
        for i in range(len(textsents)):
            if random.random() > 0.5:
                textsents[i] = DEFAULT_PJH_S_TOKEN + " " + textsents[i] + " " + DEFAULT_PJH_E_TOKEN

        example[text_column_name] = " ".join(textsents)

        if "prefix" in column_names:
            prefixsents = sent_tokenizer.tokenize(example["prefix"])
            for i in range(len(prefixsents)):
                if random.random() > 0.5:
                    prefixsents[i] = DEFAULT_PJH_S_TOKEN + " " + prefixsents[i] + " " + DEFAULT_PJH_E_TOKEN
            example["prefix"] = " ".join(prefixsents)
    ...
    # xxx: 2023-03-17
    using_anchor=donewanchor
    # using_anchor=False
    with training_args.main_process_first(desc="example per line with padding"):
        if not data_args.streaming:
            if using_anchor:
                raw_datasets = raw_datasets.map(add_ac, num_proc=data_args.preprocessing_num_workers, load_from_cache_file=not data_args.overwrite_cache,)
            ...
            if using_anchor:
                raw_datasets = raw_datasets.map(add_ac, num_proc=data_args.preprocessing_num_workers)
            lm_datasets = raw_datasets.map(
                preprocess_function,
                batched=True,
                remove_columns=column_names,
