...
def parse_config():
    ...
    parser.add_argument('--xln', type=int, default=10, help='context length during evaluation')
...
def evaluate(model, data, batch_size, device, seq_length, context_size = 4096, sliding_window=256, use_cache=False,xln=10):
    stats = {}
                ...
                if seq_length > context_size:
                    ...
                    for k in range(ctcount):
                        ...
                        if part_len == 0:
                            ...
                            # get the last n kv cache for transformer-xl
                            allkv = outputs.past_key_values
                            past_key_values = ()
                            for a,b in allkv:
                                past_key_values += ((a[:,:,-xln:,:], b[:,:,-xln:,:]),)
...
def main(args):
    ...
    if orig_ctx_len and context_size > orig_ctx_len:
        ...
        print("rope_scaling {}".format(scaling_factor))
    ...
    stats = evaluate(model, data, args.batch_size, device, args.seq_len, context_size=args.context_size, sliding_window=256, use_cache=use_cache, xln=args.context_size)
