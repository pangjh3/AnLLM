...
import nltk
nltkdir="/apdcephfs/share_733425/vinnylywang/jianhuipang/nltk_data/"
...
def main():
    ...
    if model_args.anchor_symbols is not None:
        ...
        for ans in anchor_syms:
            if ans in vocab:
                ...
                donewanchor=True
                # newanchortoken=ans
                assert len(anchor_syms) == 1 and ans == DEFAULT_PJH_TOKEN
        ...
        replace_allmtrainingforward(achorid, tokenizer.convert_tokens_to_ids(tokenizer.pad_token))
    else:
    ...
    def add_ac(example):
        ...
        example[text_column_name] = adds.join(textsents)

        if "prefix" in column_names:
            prefixsents = sent_tokenizer.tokenize(example["prefix"])
            example["prefix"] = adds.join(prefixsents)
